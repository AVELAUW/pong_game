...
class Pong(PyGameWrapper):
    ...
    def __init__(self, width=64, height=48, cpu_speed_ratio=0.6, players_speed_ratio = 0.4, ball_speed_ratio=0.75,  MAX_SCORE=11):
        ...
        self.ball_speed_ratio_start = ball_speed_ratio
    ...
    def step(self, dt):
        ...
        pygame.draw.line(self.screen, (255,255,255), [self.width/2,0], [self.width/2,self.height], 2) # Draw the net
        ...
        self.screen.blit(text, ((self.width/2)-(self.ball_radius*6),self.ball_radius) )
        text = font.render(str(self.score_counts['cpu']),1,(255,255,255))
        self.screen.blit(text, (self.width/2+(self.ball_radius),self.ball_radius) )
